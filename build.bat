...
if exist blackjack_univac.exe del /Q blackjack_univac.exe
if exist blackjack_univac.o del /Q blackjack_univac.o
if exist *.o del /Q *.o
echo.
...
echo Compiling blackjack.c...
gcc -c -DUNIVAC -O2 -Wall -Wno-unused-parameter blackjack.c -o blackjack_univac.o

if %ERRORLEVEL% NEQ 0 (
    echo ERROR: Failed to compile blackjack.c
    pause
    exit /b 1
)

echo Linking...
gcc -o blackjack_univac.exe blackjack_univac.o
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: blackjack_univac.exe

    REM Display file size
    for %%A in (blackjack_univac.exe^) do (
    ...
    echo   - BSS initialization included
    echo.
    goto :EOF
) else (
    echo.
...
REM Clean previous build artifacts
echo Cleaning previous build artifacts...
if exist blackjack.exe del /Q blackjack.exe
if exist blackjack.obj del /Q blackjack.obj
if exist *.o del /Q *.o
echo.
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: blackjack.exe

    REM Display file size
    for %%A in (blackjack.exe) do (
        echo File size: %%~zA bytes
    )
    echo.
...
cl /W4 /O2 /Fe:blackjack.exe blackjack.c
